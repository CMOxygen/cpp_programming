...
void f1(DinArr ob) //при передаче в аргумент объекта вызовется конструктор и деструктор
{
    cout << "F1 " << ob.getSize() << endl;
    ob.show();
}

void f2(DinArr &ob) //при передаче ссылки ничего вызываться не будет
{
    cout << "F2 " << ob.getSize() << endl;
    ob.show();
}
...
{
    DinArr dinarr1(5, 1);
    cout << "Array1 size " << dinarr1.getSize() << ":" << endl;
    dinarr1.show();
    ...
    DinArr dinarr5(dinarr1);
    f1(dinarr5);
    f2(dinarr5);
    cout << "Array5(dinarr1) size " << dinarr5.getSize() << ":" << endl;
    dinarr5.show();
}